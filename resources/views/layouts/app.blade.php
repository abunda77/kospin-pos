...
<head>
    ...
    <style>
        ...
        /* Ensure cart counter badge is not clipped */
        .cart-icon-container {
            position: relative;
            padding: 4px;
            overflow: visible;
...
<body class="flex flex-col min-h-screen bg-gray-100">
    <nav class="bg-white shadow-lg relative overflow-visible">
            ...
            <div class="flex justify-between items-center h-16 overflow-visible">
                <div>
                    @php
                        ...
                        $homeRoute = $isMobileMode ? route('catalog.mobile') : route('catalog');
                    @endphp
                    <a href="{{ $homeRoute }}">
                        <img src="{{ asset('images/logo_sinaraartha.png') }}" alt="Logo Sinar Artha" class="h-12">
                    </a>
                </div>
                <div class="flex items-center space-x-4 overflow-visible">
                    @php
                        $isMobileMode = session('view_preference') === 'mobile' ||
                                       request()->routeIs('catalog.mobile*') || 
                                       str_contains(request()->path(), 'm/catalog');
                        $homeRoute = $isMobileMode ? route('catalog.mobile') : route('catalog');
                    @endphp
                    <a href="{{ $homeRoute }}" class="hidden text-gray-700 sm:block hover:text-gray-900">Beranda</a>
                    ...
                    <a href="{{ route('cart') }}" class="cart-icon-container inline-flex relative items-center p-2 text-gray-700 hover:text-gray-900">
